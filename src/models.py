...
class Discriminator(nn.Module):
    def __init__(self, conv_dim=32, norm_fun='none', act_fun='LeakyReLU', use_sn=True, adv_loss_type='rahinge'):
...
class InterMerge(nn.Module):
    def __init__(self, nc_ch=64, sample_frames=5):
        super(InterMerge, self).__init__()
        use_bias = True
        ...
        self.conv14 = nn.Conv3d(nc_ch, 3, kernel_size=3, stride=1, padding=1)


    def forward(self, img):     
        
        F1 = self.relu(self.conv1(img))                               # [4, 32, 5, 256, 256]
        
        F2 = self.relu(self.conv3(self.relu(self.conv2(F1)))+F1)      # [4, 32, 5, 256, 256]
        F3 = self.relu(self.conv4(F2))                                # [4, 32, 4, 256, 256]

        F4 = self.relu(self.conv6(self.relu(self.conv5(F3)))+F3)      # [4, 32, 4, 256, 256]
        F5 = self.relu(self.conv7(F4))                                # [4, 32, 3, 256, 256]
        
        F6 = self.relu(self.conv9(self.relu(self.conv8(F5)))+F5)      # [4, 32, 3, 256, 256]
        F7 = self.relu(self.conv10(F6))                               # [4, 32, 2, 256, 256]
        
        F8 = self.relu(self.conv12(self.relu(self.conv11(F7)))+F7)    # [4, 32, 2, 256, 256]
        F9 = self.relu(self.conv13(F8))                               # [4, 32, 1, 256, 256]
         
        ## OUTPUT         
        output = self.conv14(F9)                                      # [4, 3, 1, 256, 256]
...
if __name__== '__main__':
    ...
    print(' Generator  Number of params: %.2fM' % (total / 1e6))
    print(' Generator  Number of FLOPs: %.2fGFLOPs' % (flops / 1e9))
    print(' ===================================================== ')
    ...
    print(' Discriminator  Number of params: %.2fM' % (total / 1e6))
    print(' Discriminator  Number of FLOPs: %.2fGFLOPs' % (flops / 1e9))
    print(' ===================================================== ')
    
    
    net = InterMerge().cuda()
    input = torch.randn(20, 3, 256, 256).cuda()
    flops, params = profile(net, inputs=(input,))
    total = sum([param.nelement() for param in net.parameters()])
    
    print(' InterMerge  Number of params: %.2fM' % (total / 1e6))
    print(' InterMerge  Number of FLOPs: %.2fGFLOPs' % (flops / 1e9))
